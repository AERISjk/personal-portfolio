...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  const animateText = useCallback((opening: boolean) => {
    ...
    gsap.set(wrap, { "--sm-toggle-width": `${currentWidth}px` });
    ...
    textCycleAnimRef.current = gsap.to(inner, {
      ...
      ease: "power4.out",
      onComplete: () => {
        gsap.set(wrap, { clearProps: "--sm-toggle-width" });
      },
    });
  }, []);
  ...
  return (
    // 🔥 FIX 1: Conditionally apply pointer-events-none when the menu is CLOSED.
    <div
      className={`sm-scope z-40 ${
        isFixed
          ? "fixed top-0 left-0 w-screen h-screen overflow-hidden"
          : "w-full h-full"
      } ${!open ? "pointer-events-none-override" : ""}`}>
      <div
        className={
          (className ? className + " " : "") +
        ...
        <aside
          ...
          ref={panelRef}
          className="staggered-menu-panel absolute top-0 right-0 h-full bg-white flex flex-col p-[6em_2em_2em_2em] overflow-y-auto z-10 backdrop-blur-[12px] pointer-events-none"
          style={{ WebkitBackdropFilter: "blur(12px)" }}
          aria-hidden={!open}>
          <div className="sm-panel-inner flex-1 flex flex-col gap-5 pointer-events-auto">
            <ul
              className="sm-panel-list list-none m-0 p-0 flex flex-col gap-2 pointer-events-auto"
              role="list"
            ...
            {displaySocials && socialItems && socialItems.length > 0 && (
              <div
                className="sm-socials mt-auto pt-8 flex flex-col gap-3 pointer-events-auto"
      ...
      <style>{`
/* --- SCROLL/CLICK FIXES --- */
/* 🔥 FIX 2: CSS to prevent scrolling on the main body when the menu is open */
.sm-no-scroll {
    overflow: hidden !important;
    touch-action: none;
}

/* 🔥 FIX 1: CSS to allow clicks to pass through the full-screen wrapper when the menu is CLOSED */
.sm-scope.pointer-events-none-override { 
    pointer-events: none; 
}
/* Ensure the button/logo remain clickable even when the wrapper has pointer-events: none */
.sm-scope.pointer-events-none-override .sm-toggle,
.sm-scope.pointer-events-none-override .sm-logo { 
    pointer-events: auto; 
}
/* --- END FIXES --- */
...
.sm-scope .sm-panel-inner { flex: 1; display: flex; flex-direction: column; gap: 1.25rem; pointer-events: auto; }
